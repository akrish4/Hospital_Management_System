...
local_server= True
app = Flask(__name__)
app.secret_key='ananthakrishnan'
...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/dp_hms'
db=SQLAlchemy(app)
...
def contacts():
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `contacts` (`email`,`name`,`description`) VALUES ('{email}','{name}','{description}')")
        flash("Information is Stored , Thank you for contacting us  ","primary")
...
@app.route('/doctorsdetail')
def doctorsdetail():  
    query=db.engine.execute(f"SELECT * FROM `doctors`")
    return render_template('doctorsdetail.html',query=query)

@app.route('/patientsdetail')
def patientsdetail():  
    query=db.engine.execute(f"SELECT * FROM `patients`")
    return render_template('patientsdetail.html',query=query)


@app.route('/userdetail')
def userdetail():  
    query=db.engine.execute(f"SELECT * FROM `user`")
    return render_template('userdetail.html',query=query)
...
def signup():
    if request.method == "POST":
        ...
        new_user=db.engine.execute(f"INSERT INTO `user` (`username`,`email`,`password`, `role`) VALUES ('{username}','{email}','{encpassword}','{role}')")
